...
"load" === app_vars.counter_start && (counter_start_object = window), $(counter_start_object).on(app_vars.counter_start + ".adLinkFly.checkAdblockers", function(t) {
    ...
    if ("interstitial" === ad_type) {
        ...
        window.setTimeout(function() {
            ...
            t = setInterval(function() {
                e -= 1e3, i.text(e / 1e3 + " s"), e <= 0 && (n.html('<a href="" class="btn" rel="noreferrer" onclick="javascript: return false;">' + app_vars.skip_ad + "</a>"), clearInterval(t), $("#go-link").addClass("go-link"), $("#go-link.go-link").submit())
            }, 1e3)
        }, 500)
    }
