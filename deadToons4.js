...
$("#mainNav.navbar-fixed-top").affix({
    ...
    "solvemedia" === app_vars.captcha_type && ("" === app_vars.user_id && "1" === app_vars.captcha_short_anonymous && $("#captchaShort").length && (captchaShort = ACPuzzle.create(app_vars.solvemedia_challenge_key, "captchaShort", {
        multi: !0,
        id: "captchaShort"
...
var onloadRecaptchaCallback = function() {
    if ("yes" !== app_vars.enable_captcha) return !0;
    "recaptcha" === app_vars.captcha_type && ("" === app_vars.user_id && "1" === app_vars.captcha_short_anonymous && $("#captchaShort").length && ($("#shorten .btn-captcha").attr("disabled", "disabled"), captchaShort = grecaptcha.render("captchaShort", {
    ...
    }))), "invisible-recaptcha" === app_vars.captcha_type && ("" === app_vars.user_id && "1" === app_vars.captcha_short_anonymous && $("#captchaShort").length && (invisibleCaptchaShort = grecaptcha.render("captchaShort", {
...
function checkAdblockUser() {
    if ("1" !== getCookie("ab") && (document.cookie = "ab=2; path=/", !document.getElementById("test-block") && (document.cookie = "ab=1; path=/", "1" === app_vars.force_disable_adblock))) {
        var t = '<div class="alert alert-danger" style="display: inline-block;">' + app_vars.please_disable_adblock + "</div>";
        $("#link-view").replaceWith(t), $(".banner-page a.get-link").replaceWith(t), $(".interstitial-page div.skip-ad").replaceWith(t)
    }
}
...
"load" === app_vars.counter_start && (counter_start_object = window), $(counter_start_object).on(app_vars.counter_start + ".adLinkFly.checkAdblockers", function(t) {
    ...
    if ("banner" === ad_type) {
        ...
        window.setTimeout(function() {
            var t, n = 1 * app_vars.counter_value;
            t = setInterval(function() {
                window.blurred || (n -= 1, e.text(n / 1), n <= 0 && (clearInterval(t), $("#go-link").addClass("go-link"), $("#go-link.go-link").submit()))
            }, 1)
    ...
    if ("interstitial" === ad_type) {
        var n = $(".skip-ad"),
            i = $(".skip-ad .counter");
        window.setTimeout(function() {
            var t, e = 1 * app_vars.counter_value;
            t = setInterval(function() {
                e -= 1, i.text(e / 1 + " s"), e <= 0 && (n.html('<a href="" class="btn" rel="noreferrer" onclick="javascript: return false;">' + app_vars.skip_ad + "</a>"), clearInterval(t), $("#go-link").addClass("go-link"), $("#go-link.go-link").submit())
            }, 1)
        }, 500)
    }
    ...
    if (e.hasClass("go-link")) {
        ...
        $.ajax({
            ...
            success: function(t, e, n) {
                t.url ? ("banner" === ad_type && $("a.get-link").attr("href", t.url).removeClass("disabled").text(app_vars.get_link), "interstitial" === ad_type && $(".skip-ad a").attr("href", t.url).removeAttr("onclick")) : alert(t.message)
            },
            error: function(t, e, n) {
...
$("#form-settings").length && null !== selectedTab ? $('#form-settings a[data-toggle="tab"][href="#' + selectedTab + '"]').tab("show") : $('#form-settings a[data-toggle="tab"]:first').tab("show"), $(".shorten-member #shorten").on("submit.adLinkFly.memberShortLinkForm", function(t) {
    ...
    if (t.preventDefault(), "" != app_vars.user_id || "yes" !== app_vars.home_shortening_register) {
        ...
        $.ajax({
            ...
            success: function(t, n, i) {
                if (t.url) {
                    ...
                    $(".shorten.add-link-result").html(s).slideDown()
                }
            },
            error: function(t, e, n) {
                alert("An error occured: " + t.status + " " + t.statusText)
